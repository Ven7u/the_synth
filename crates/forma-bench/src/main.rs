...
//!
//! Usage:
//!   cargo run -p forma-bench              # run all suites
//!   cargo run -p forma-bench -- reverb    # run matching suites only

use forma_dsp::{PeakLimiter, ShimmerReverb};
...
fn check_reverb(rev_type: u8, room: f32, damp: f32, shimmer_amt: f32, pitch: u8) {
    ...
    let metrics = measure_unit(
        |inp| rev.tick(inp, room, damp, shimmer_amt, pitch, rev_type),
        90_000,
        5_000,
    );
...
fn check_limiter(drive: f32, threshold: f32, expected_ceil: f32, label: &str) {
    ...
    assert!(
        max_out <= expected_ceil,
        "{label}: peak {max_out:.3} > {expected_ceil:.3}"
    );
}
