//! Multi-track ambient engine.
//!
//! Four independent tracks, each a full 6-voice subtractive synth voice bank.
//! All tracks feed into two global effect buses (shimmer and crystal) via
//! per-track send levels. The buses are dry pass-through until Phase 5.
//!
//! Design invariants (inherited from synth-engine):
...
/// Mirrors AudioState from synth-engine plus two send levels.
pub struct TrackState {
    ...
    pub track_vol: Shared,  // per-track volume (0.0..1.0)
...
/// Build the 6-voice poly graph for one track.
/// Returns a mono signal (pre-send, pre-mix).
...
// ---------------------------------------------------------------------------
// AmbientEngine — four tracks + global buses
// ---------------------------------------------------------------------------

/// The multi-track ambient engine.
///
/// Holds 4 tracks and two placeholder global buses (shimmer, crystal).
/// The audio callback calls `process_stereo` each buffer to get interleaved stereo output.
pub struct AmbientEngine {
    ...
    // Internal: one BlockRateAdapter per track graph
    track_graphs: Vec<BlockRateAdapter>,
    sr: f64,

    // Glide smoothing state per track (one per voice per track)
...
impl AmbientEngine {
    ...
    /// Call once per audio buffer, before calling `process_stereo`.
    ...
    /// Get one stereo sample pair summed from all four tracks.
    /// The caller is responsible for calling `tick_glide` and `tick_lfo_sample` at the right rate.
    #[inline]
    pub fn get_stereo(&mut self) -> (f32, f32) {
        ...
        // Normalize by track count, apply master vol, soft clip
